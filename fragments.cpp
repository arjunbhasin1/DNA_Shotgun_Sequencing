...
std::vector<std::string> load_fragments (const std::string& filename)
{
  ...
  std::vector<std::string> fragments; //stores fragemnts as a string of vectors
  std::string frag; //prints the fragment data into the string called frag 
  while (infile >> frag) //reads all DNA fragments and NOT stop when new line is reached - use a while loop - keeps pulling lines of data out of file until file is done
